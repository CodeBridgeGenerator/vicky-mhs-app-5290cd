...
const _setLoginEmail = async (email, access) => {
    const getUserLogin = await client.service('userLogin').find({ query: { loginEmail: email } });
    if (getUserLogin?.data?.length === 0) {
        await client.service('userLogin').create({ loginEmail: email, status: true, access: access });
    } else {
        const id = getUserLogin?.data[0]?._id;
        await client.service('userLogin').patch(id, { status: true, access: access });
    }
};

const _setLogOutEmail = async (email) => {
    const getUserLogin = await client.service('userLogin').find({ query: { loginEmail: email } });
    if (getUserLogin?.data?.length === 0) {
        await client.service('userLogin').create({ loginEmail: email, status: false, access: null });
    } else {
        const id = getUserLogin?.data[0]?._id;
        await client.service('userLogin').patch(id, { status: false, access: null });
    }
};
        ...
        async loginForOAuth(data, reduxState) {
            return new Promise(async (resolve, reject) => {
                ...
                try {
                    ...
                    dispatch.toast.alert({
                        type: 'error',
                        message: 'First, you have to sign up!'
                    });
                    reject(error);
                }
                dispatch.loading.hide();
        ...
        async logout(_, reduxState) {
            ...
            await client
                ...
                .then(async (res) => {
                    ...
                    await _setLogOutEmail(user.email);
                })
                .catch((error) => {
                    console.log('error', { error });
                    dispatch.toast.alert({
