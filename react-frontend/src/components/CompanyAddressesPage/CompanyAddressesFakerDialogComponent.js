...
import companyAddressesFakerFactory from './companyAddressesFakerFactory';
...
const CompanyAddressesFakerDialogComponent = (props) => {
    ...
    const onRun = async () => {
        let fakeData = companyAddressesFakerFactory(props.user, fakerCount, companyIdIds);

        setLoading(true);
        const promises = fakeData.map((elem) => client.service('companyAddresses').create(elem));
...
export default connect(mapState, mapDispatch)(CompanyAddressesFakerDialogComponent);
