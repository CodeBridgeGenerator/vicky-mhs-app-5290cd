...
import seederData from './companyAddresses.seeder.json';
...
const CompanyAddressesSeederDialogComponent = (props) => {
    ...
    const onRun = async () => {
        ...
        try {
            for (let i = 0; i < seederData.length; i++) {
                ...
                const res = await client.service('companyAddresses').create(_data);
                results = [...results, res];
            }
            props.onHide();
            props.alert({
...
export default connect(mapState, mapDispatch)(CompanyAddressesSeederDialogComponent);
