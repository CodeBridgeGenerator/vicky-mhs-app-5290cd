...
const CompanyAddressesPage = (props) => {
    ...
    useEffect(() => {
        let service = _.find(config.services, { serviceName: 'companyAddresses' });
        if (!service) {
    ...
    return (
        <div className="mt-5">
            ...
            <CompanyAddressesEditDialogComponent entity={_.find(data, { _id: selectedEntityIndex })} show={showEditDialog} onHide={() => setShowEditDialog(false)} onEditResult={onEditResult} />
            <CompanyAddressesCreateDialogComponent entity={newRecord} show={showCreateDialog} onHide={() => setShowCreateDialog(false)} onCreateResult={onCreateResult} />
