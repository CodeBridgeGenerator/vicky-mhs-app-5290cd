...
import departmentAdminFakerFactory from './departmentAdminFakerFactory';
...
const DepartmentAdminFakerDialogComponent = (props) => {
    const [fakerCount, setFakerCount] = useState(1);
    const [errors, setErrors] = useState([]);
    const [loading, setLoading] = useState(false);
    const [departmentIdIds, setdepartmentIdIds] = useState();
    ...
    const onRun = async () => {
        let fakeData = departmentAdminFakerFactory(props.user, fakerCount, departmentIdIds, employeeIdIds);

        setLoading(true);
        const promises = fakeData.map((elem) => client.service('departmentAdmin').create(elem));
...
export default connect(mapState, mapDispatch)(DepartmentAdminFakerDialogComponent);
