...
import seederData from './departmentAdmin.seeder.json';
...
const DepartmentAdminSeederDialogComponent = (props) => {
    ...
    const onRun = async () => {
        ...
        try {
            for (let i = 0; i < seederData.length; i++) {
                ...
                const res = await client.service('departmentAdmin').create(_data);
                results = [...results, res];
            }
            props.onHide();
            props.alert({
...
export default connect(mapState, mapDispatch)(DepartmentAdminSeederDialogComponent);
