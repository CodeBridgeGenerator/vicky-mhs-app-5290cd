...
const DepartmentAdminPage = (props) => {
        ...
        client
            ...
            .catch((error) => {
                ...
                props.alert({
                    ...
                    type: 'error',
                    message: error.message || 'Failed get Department Admins'
                });
            });

        const service = _.find(config.services, { serviceName: 'departmentAdmin' });
        const fields =
    ...
    const deleteAll = async () => {
        ...
        const promises = data.map((e) => client.service('departmentAdmin').remove(e._id));
        await Promise.all(
            promises.map((p) =>
                p.catch((error) => {
                    props.alert({
        ...
        <div className="mt-5">
            <div className="grid">
                <div className="col-6 flex justify-content-start">
                    <h3 className="mb-0 ml-2">Department Admins </h3>
                </div>
                <div className="col-6 flex justify-content-end">
                    <>
                        <Button label="add" loading={loading} icon="pi pi-plus" onClick={() => setShowCreateDialog(true)} role="departmentAdmin-add-button" />
            ...
            <div className="grid align-items-center">
                <div className="col-12" role="departmentAdmin-datatable">
            ...
            <DepartmentAdminEditDialogComponent entity={_.find(data, { _id: selectedEntityIndex })} show={showEditDialog} onHide={() => setShowEditDialog(false)} onEditResult={onEditResult} />
            <DepartmentAdminCreateDialogComponent entity={newRecord} show={showCreateDialog} onHide={() => setShowCreateDialog(false)} onCreateResult={onCreateResult} />
            <DepartmentAdminFakerDialogComponent show={showFakerDialog} onHide={() => setShowFakerDialog(false)} onFakerCreateResults={onFakerCreateResults} />
            <DepartmentAdminSeederDialogComponent show={showSeederDialog} onHide={() => setShowSeederDialog(false)} onSeederResults={onSeederResults} />
