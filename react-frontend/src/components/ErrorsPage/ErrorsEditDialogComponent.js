...
const ErrorsCreateDialogComponent = (props) => {
    ...
    const onSave = async () => {
        ...
        setLoading(true);
        try {
            const result = await client.service('errors').patch(_entity._id, _data);
            props.onHide();
            props.alert({
                type: 'success',
                title: 'Edit info',
                message: 'Info errors updated successfully'
            });
    ...
    return (
        <Dialog header="Edit Errors" visible={props.show} closable={false} onHide={props.onHide} modal style={{ width: '40vw' }} className="min-w-max" footer={renderFooter()} resizable={false}>
            <div className="grid p-fluid overflow-y-auto" style={{ maxWidth: '55vw' }} role="errors-edit-dialog-component">
