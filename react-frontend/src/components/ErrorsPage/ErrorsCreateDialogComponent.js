...
const ErrorsCreateDialogComponent = (props) => {
    ...
    const onSave = async () => {
        ...
        try {
            const result = await client.service('errors').create(_data);
            props.onHide();
            props.alert({
                type: 'success',
                title: 'Create info',
                message: 'Info Errors created successfully'
            });
            ...
            props.alert({
                type: 'error',
                title: 'Create',
                message: 'Failed to create in Errors'
            });
        }
        setLoading(false);
    };
    ...
    return (
        <Dialog header="Create Errors" visible={props.show} closable={false} onHide={props.onHide} modal style={{ width: '40vw' }} className="min-w-max" footer={renderFooter()} resizable={false}>
            <div className="grid p-fluid overflow-y-auto" style={{ maxWidth: '55vw' }} role="errors-create-dialog-component">
                ...
                <div className="col-12 md:col-6 field mt-5">
                    <span className="align-items-center">
                        ...
                        <InputText id="error" className="w-full mb-3 p-inputtext-sm" value={_entity?.error} onChange={(e) => setValByKey('error', e.target.value)} />
                    </span>
                    <small className="p-error">
...
export default connect(mapState, mapDispatch)(ErrorsCreateDialogComponent);
