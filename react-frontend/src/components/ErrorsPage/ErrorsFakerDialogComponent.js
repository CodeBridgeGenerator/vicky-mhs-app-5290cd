...
import errorsFakerFactory from './errorsFakerFactory';
...
const ErrorsFakerDialogComponent = (props) => {
    ...
    const onRun = async () => {
        let fakeData = errorsFakerFactory(props.user, fakerCount);

        setLoading(true);
        const promises = fakeData.map((elem) => client.service('errors').create(elem));
...
export default connect(mapState, mapDispatch)(ErrorsFakerDialogComponent);
