...
import ErrorsFakerDialogComponent from './ErrorsFakerDialogComponent';
import ErrorsSeederDialogComponent from './ErrorsSeederDialogComponent';
...
const ErrorsPage = (props) => {
        ...
        client
            ...
            .catch((error) => {
                ...
                props.alert({
                    title: 'Errors',
                    type: 'error',
                    message: error.message || 'Failed get Errors'
                });
            });

        const service = _.find(config.services, { serviceName: 'errors' });
        const fields =
    ...
    const deleteRow = async () => {
        try {
            await client.service('errors').remove(selectedEntityIndex);
    ...
    const deleteAll = async () => {
        ...
        const promises = data.map((e) => client.service('errors').remove(e._id));
        await Promise.all(
            promises.map((p) =>
                p.catch((error) => {
                    props.alert({
        ...
        <div className="mt-5">
            <div className="grid">
                <div className="col-6 flex justify-content-start">
                    <h3 className="mb-0 ml-2">Errors </h3>
                </div>
                <div className="col-6 flex justify-content-end">
                    <>
                        <Button label="add" loading={loading} icon="pi pi-plus" onClick={() => setShowCreateDialog(true)} role="errors-add-button" disabled={true} />
            ...
            <div className="grid align-items-center">
                <div className="col-12" role="errors-datatable">
                    <ErrorsDatatable
                        items={data}
                        fields={fields}
            ...
            <ErrorsEditDialogComponent entity={_.find(data, { _id: selectedEntityIndex })} show={showEditDialog} onHide={() => setShowEditDialog(false)} onEditResult={onEditResult} />
            <ErrorsCreateDialogComponent entity={newRecord} show={showCreateDialog} onHide={() => setShowCreateDialog(false)} onCreateResult={onCreateResult} />
            <ErrorsFakerDialogComponent show={showFakerDialog} onHide={() => setShowFakerDialog(false)} onFakerCreateResults={onFakerCreateResults} />
            <ErrorsSeederDialogComponent show={showSeederDialog} onHide={() => setShowSeederDialog(false)} onSeederResults={onSeederResults} />
...
export default connect(mapState, mapDispatch)(ErrorsPage);
