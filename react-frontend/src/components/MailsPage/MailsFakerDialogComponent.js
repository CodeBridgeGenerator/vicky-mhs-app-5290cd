...
import mailsFakerFactory from './mailsFakerFactory';
...
const MailsFakerDialogComponent = (props) => {
    ...
    const onRun = async () => {
        let fakeData = mailsFakerFactory(props.user, fakerCount);

        setLoading(true);
        const promises = fakeData.map((elem) => client.service('mails').create(elem));
...
export default connect(mapState, mapDispatch)(MailsFakerDialogComponent);
