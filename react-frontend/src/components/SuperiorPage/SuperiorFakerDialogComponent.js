...
import superiorFakerFactory from './superiorFakerFactory';
...
const SuperiorFakerDialogComponent = (props) => {
    ...
    const onRun = async () => {
        let fakeData = superiorFakerFactory(props.user, fakerCount);

        setLoading(true);
        const promises = fakeData.map((elem) => client.service('superior').create(elem));
...
export default connect(mapState, mapDispatch)(SuperiorFakerDialogComponent);
