...
import inboxFakerFactory from './inboxFakerFactory';
...
const InboxFakerDialogComponent = (props) => {
    ...
    const onRun = async () => {
        let fakeData = inboxFakerFactory(props.user, fakerCount, fromIds, toUserIds);

        setLoading(true);
        const promises = fakeData.map((elem) => client.service('inbox').create(elem));
...
export default connect(mapState, mapDispatch)(InboxFakerDialogComponent);
