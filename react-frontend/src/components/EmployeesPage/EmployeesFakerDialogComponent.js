...
import employeesFakerFactory from './employeesFakerFactory';
...
const EmployeesFakerDialogComponent = (props) => {
    ...
    useEffect(() => {
        ...
        client
            ...
            .find({ query: { $select: ['_id', 'name'] } })
            .then((data) => {
                setsupervisorIds(data.data);
            });
    }, [props.show]);

    const onRun = async () => {
        let fakeData = employeesFakerFactory(props.user, fakerCount, companyIds, departmentIds, sectionIds, positionIds, supervisorIds);

        setLoading(true);
        const promises = fakeData.map((elem) => client.service('employees').create(elem));
...
export default connect(mapState, mapDispatch)(EmployeesFakerDialogComponent);
