...
import userLoginFakerFactory from './userLoginFakerFactory';
...
const UserLoginFakerDialogComponent = (props) => {
    ...
    const onRun = async () => {
        let fakeData = userLoginFakerFactory(props.user, fakerCount);

        setLoading(true);
        const promises = fakeData.map((elem) => client.service('userLogin').create(elem));
...
export default connect(mapState, mapDispatch)(UserLoginFakerDialogComponent);
