...
import promptsFakerFactory from './promptsFakerFactory';
...
const PromptsFakerDialogComponent = (props) => {
    const [fakerCount, setFakerCount] = useState(1);
    const [errors, setErrors] = useState([]);
    const [loading, setLoading] = useState(false);

    const [chataiidIds, setchataiidIds] = useState();

    const [configidIds, setconfigidIds] = useState();
    ...
    useEffect(() => {
        ...
        client
            ...
            .then((data) => {
                setchataiidIds(data.data);
            });
        client
            .service('config')
            .find({ query: { $select: ['_id', 'name'] } })
            .then((data) => {
                setconfigidIds(data.data);
            });
    }, [props.show]);

    const onRun = async () => {
        let fakeData = promptsFakerFactory(fakerCount, chataiidIds, configidIds);

        setLoading(true);
        const promises = fakeData.map((elem) => client.service('prompts').create(elem));
...
export default connect(mapState, mapDispatch)(PromptsFakerDialogComponent);
