...
import templatesFakerFactory from './templatesFakerFactory';
...
const TemplatesFakerDialogComponent = (props) => {
    ...
    const onRun = async () => {
        let fakeData = templatesFakerFactory(props.user, fakerCount);

        setLoading(true);
        const promises = fakeData.map((elem) => client.service('templates').create(elem));
...
export default connect(mapState, mapDispatch)(TemplatesFakerDialogComponent);
