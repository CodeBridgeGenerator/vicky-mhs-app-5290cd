...
import userInvitesFakerFactory from './userInvitesFakerFactory';
...
const UserInvitesFakerDialogComponent = (props) => {
    ...
    const onRun = async () => {
        let fakeData = userInvitesFakerFactory(props.user, fakerCount);

        setLoading(true);
        const promises = fakeData.map((elem) => client.service('userInvites').create(elem));
...
export default connect(mapState, mapDispatch)(UserInvitesFakerDialogComponent);
