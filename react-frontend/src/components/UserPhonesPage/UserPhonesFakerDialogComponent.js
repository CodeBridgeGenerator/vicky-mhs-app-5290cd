...
import userPhonesFakerFactory from './userPhonesFakerFactory';
...
const UserPhonesFakerDialogComponent = (props) => {
    ...
    const onRun = async () => {
        let fakeData = userPhonesFakerFactory(props.user, fakerCount, userIdIds);

        setLoading(true);
        const promises = fakeData.map((elem) => client.service('userPhones').create(elem));
...
export default connect(mapState, mapDispatch)(UserPhonesFakerDialogComponent);
