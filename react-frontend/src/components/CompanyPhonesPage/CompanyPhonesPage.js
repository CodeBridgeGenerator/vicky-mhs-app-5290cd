...
const CompanyPhonesPage = (props) => {
    ...
    useEffect(() => {
        let service = _.find(config.services, { serviceName: 'companyPhones' });
        if (!service) {
            service = _.find(standard.services, { serviceName: 'companyPhones' });
        }
    ...
    useEffect(() => {
        ...
        const service = _.find(config.services, { serviceName: 'companyPhones' });
        const fields =
    ...
    const deleteAll = async () => {
        ...
        const promises = data.map((e) => client.service('companyPhones').remove(e._id));
        await Promise.all(
            promises.map((p) =>
                p.catch((error) => {
                    props.alert({
        ...
        <div className="mt-5">
            <div className="grid">
                <div className="col-6 flex justify-content-start">
                    <h3 className="mb-0 ml-2">Company Phones </h3>
                </div>
                <div className="col-6 flex justify-content-end">
                    <>
                        <Button label="add" icon="pi pi-plus" onClick={() => setShowCreateDialog(true)} role="companyPhones-add-button" />
            ...
            <CompanyPhonesEditDialogComponent entity={_.find(data, { _id: selectedEntityIndex })} show={showEditDialog} onHide={() => setShowEditDialog(false)} onEditResult={onEditResult} />
            <CompanyPhonesCreateDialogComponent entity={newRecord} show={showCreateDialog} onHide={() => setShowCreateDialog(false)} onCreateResult={onCreateResult} />
            <CompanyPhonesFakerDialogComponent show={showFakerDialog} onHide={() => setShowFakerDialog(false)} onFakerCreateResults={onFakerCreateResults} />
            <CompanyPhonesSeederDialogComponent show={showSeederDialog} onHide={() => setShowSeederDialog(false)} onSeederResults={onSeederResults} />
