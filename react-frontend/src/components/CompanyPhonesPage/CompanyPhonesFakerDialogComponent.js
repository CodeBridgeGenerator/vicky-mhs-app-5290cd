...
import companyPhonesFakerFactory from './companyPhonesFakerFactory';
...
const CompanyPhonesFakerDialogComponent = (props) => {
    ...
    const onRun = async () => {
        let fakeData = companyPhonesFakerFactory(props.user, fakerCount, companyIdIds);

        setLoading(true);
        const promises = fakeData.map((elem) => client.service('companyPhones').create(elem));
...
export default connect(mapState, mapDispatch)(CompanyPhonesFakerDialogComponent);
