...
import seederData from './companyPhones.seeder.json';
...
const CompanyPhonesSeederDialogComponent = (props) => {
    ...
    const onRun = async () => {
        ...
        try {
            for (let i = 0; i < seederData.length; i++) {
                ...
                const res = await client.service('companyPhones').create(_data);
                results = [...results, res];
            }
            props.onHide();
            props.alert({
...
export default connect(mapState, mapDispatch)(CompanyPhonesSeederDialogComponent);
