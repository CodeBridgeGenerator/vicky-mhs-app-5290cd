...
import MailWHDatatable from './MailWHDataTable';
...
import MailWHFakerDialogComponent from './MailWHFakerDialogComponent';
import MailWHSeederDialogComponent from './MailWHSeederDialogComponent';

const MailWHPage = (props) => {
        ...
        client
            ...
            .catch((error) => {
                ...
                props.alert({
                    title: 'MailWH',
                    type: 'error',
                    message: error.message || 'Failed get MailWH'
                });
            });

        const service = _.find(config.services, { serviceName: 'mailWH' });
        const fields =
    ...
    const deleteRow = async () => {
        try {
            await client.service('mailWH').remove(selectedEntityIndex);
    ...
    const deleteAll = async () => {
        ...
        const promises = data.map((e) => client.service('mailWH').remove(e._id));
        await Promise.all(
            promises.map((p) =>
                p.catch((error) => {
                    props.alert({
        ...
        <div className="mt-5">
            <div className="grid">
                <div className="col-6 flex justify-content-start">
                    <h3 className="mb-0 ml-2">Mail Warehouse </h3>
                </div>
                <div className="col-6 flex justify-content-end">
                    <>
                        <Button label="add" loading={loading} icon="pi pi-plus" onClick={() => setShowCreateDialog(true)} role="mailWH-add-button" disabled={true} />
            ...
            <div className="grid align-items-center">
                <div className="col-12" role="mailWH-datatable">
                    <MailWHDatatable
                        items={data}
                        fields={fields}
            ...
            <MailWHEditDialogComponent entity={_.find(data, { _id: selectedEntityIndex })} show={showEditDialog} onHide={() => setShowEditDialog(false)} onEditResult={onEditResult} />
            <MailWHCreateDialogComponent entity={newRecord} show={showCreateDialog} onHide={() => setShowCreateDialog(false)} onCreateResult={onCreateResult} />
            <MailWHFakerDialogComponent show={showFakerDialog} onHide={() => setShowFakerDialog(false)} onFakerCreateResults={onFakerCreateResults} />
            <MailWHSeederDialogComponent show={showSeederDialog} onHide={() => setShowSeederDialog(false)} onSeederResults={onSeederResults} />
...
export default connect(mapState, mapDispatch)(MailWHPage);
