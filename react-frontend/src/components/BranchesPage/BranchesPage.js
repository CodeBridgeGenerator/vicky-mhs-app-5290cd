...
const BranchesPage = (props) => {
    ...
    useEffect(() => {
        let service = _.find(config.services, { serviceName: 'branches' });
        if (!service) {
            service = _.find(standard.services, { serviceName: 'branches' });
        }
    ...
    useEffect(() => {
        ...
        const service = _.find(config.services, { serviceName: 'branches' });
        const fields =
        ...
        <div className="mt-5">
            <div className="grid">
                ...
                <div className="col-6 flex justify-content-end">
                    <>
                        <Button label="add" icon="pi pi-plus" onClick={() => setShowCreateDialog(true)} role="branches-add-button" />
            ...
            <BranchesEditDialogComponent entity={_.find(data, { _id: selectedEntityIndex })} show={showEditDialog} onHide={() => setShowEditDialog(false)} onEditResult={onEditResult} />
            <BranchesCreateDialogComponent entity={newRecord} show={showCreateDialog} onHide={() => setShowCreateDialog(false)} onCreateResult={onCreateResult} />
            <BranchesFakerDialogComponent show={showFakerDialog} onHide={() => setShowFakerDialog(false)} onFakerCreateResults={onFakerCreateResults} />
            <BranchesSeederDialogComponent show={showSeederDialog} onHide={() => setShowSeederDialog(false)} onSeederResults={onSeederResults} />
