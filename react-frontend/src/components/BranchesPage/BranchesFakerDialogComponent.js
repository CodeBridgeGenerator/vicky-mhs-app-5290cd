...
import branchesFakerFactory from './branchesFakerFactory';
...
const BranchesFakerDialogComponent = (props) => {
    ...
    const onRun = async () => {
        let fakeData = branchesFakerFactory(props.user, fakerCount, companyIdIds);

        setLoading(true);
        const promises = fakeData.map((elem) => client.service('branches').create(elem));
...
export default connect(mapState, mapDispatch)(BranchesFakerDialogComponent);
