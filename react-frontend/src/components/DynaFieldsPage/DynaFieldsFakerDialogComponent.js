...
import dynaFieldsFakerFactory from './dynaFieldsFakerFactory';
...
const DynaFieldsFakerDialogComponent = (props) => {
    const [fakerCount, setFakerCount] = useState(1);
    const [errors, setErrors] = useState([]);
    const [loading, setLoading] = useState(false);
    const [dynaLoaderIds, setdynaLoaderIds] = useState();
    ...
    useEffect(() => {
        ...
        client
            ...
            .then((data) => {
                setdynaLoaderIds(data.data);
            });
    }, [props.show]);

    const onRun = async () => {
        let fakeData = dynaFieldsFakerFactory(props.user, fakerCount, dynaLoaderIds);

        setLoading(true);
        const promises = fakeData.map((elem) => client.service('dynaFields').create(elem));
...
export default connect(mapState, mapDispatch)(DynaFieldsFakerDialogComponent);
