...
import DynaFieldsEditDialogComponent from './DynaFieldsEditDialogComponent';
...
const DynaFieldsPage = (props) => {
    ...
    const filename = 'dynaFields.csv';

    useEffect(() => {
        let service = _.find(config.services, { serviceName: 'dynaFields' });
        if (!service) {
            service = _.find(standard.services, { serviceName: 'dynaFields' });
        }
        setSelectedHideFields(service?.schemaList?.map((f, i) => (i > 5 && typeof f !== 'array' ? f.fieldName : null)));
    }, []);
    ...
    const deleteRow = async () => {
        try {
            await client.service('dynaFields').remove(selectedEntityIndex);
    ...
    const deleteAll = async () => {
        ...
        const promises = data.map((e) => client.service('dynaFields').remove(e._id));
        await Promise.all(
            promises.map((p) =>
                p.catch((error) => {
                    props.alert({
        ...
        <div className="mt-5">
            <div className="grid">
                ...
                <div className="col-6 flex justify-content-end">
                    <Button label="add" icon="pi pi-plus" onClick={() => setShowCreateDialog(true)} role="dynaFields-add-button" disabled={true} />
                    <SplitButton model={menuItems.filter((m) => !(m.icon === 'pi pi-trash' && data?.length === 0))} dropdownIcon="pi pi-ellipsis-v" buttonClassName="hidden" menuButtonClassName="ml-1 p-button-text" disabled={true}></SplitButton>
                </div>
            </div>
            ...
            <DynaFieldsEditDialogComponent service={props.service} entity={_.find(data, { _id: selectedEntityIndex })} show={showEditDialog} onHide={() => setShowEditDialog(false)} onEditResult={onEditResult} />
            <DynaFieldsCreateDialogComponent service={props.service} entity={newRecord} show={showCreateDialog} onHide={() => setShowCreateDialog(false)} onCreateResult={onCreateResult} />
            <DynaFieldsFakerDialogComponent show={showFakerDialog} onHide={() => setShowFakerDialog(false)} onFakerCreateResults={onFakerCreateResults} />
            <DynaFieldsSeederDialogComponent show={showSeederDialog} onHide={() => setShowSeederDialog(false)} onSeederResults={onSeederResults} />
