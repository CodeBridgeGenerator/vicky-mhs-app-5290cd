...
import DynaLoaderFakerDialogComponent from './DynaLoaderFakerDialogComponent';
import DynaLoaderSeederDialogComponent from './DynaLoaderSeederDialogComponent';
...
const DynaLoaderPage = (props) => {
    ...
    const deleteAll = async () => {
        ...
        const promises = data.map((e) => client.service('dynaLoader').remove(e._id));
        await Promise.all(
            promises.map((p) =>
                p.catch((error) => {
                    props.alert({
    ...
    const deleteDynaFieldsAll = async (id) => {
        const dynaFields = await client.service('dynaFields').find({ query: { dynaLoader: id } });
        const promises = dynaFields?.data?.map((e) => client.service('dynaFields').remove(e._id));
        await Promise.all(
            promises.map((p) =>
                p.catch((error) => {
                    props.alert({
        ...
        <div className="mt-5">
            <div className="grid">
                <div className="col-6 flex justify-content-start">
                    <h3 className="mb-0 ml-2">DynaLoader </h3>
                </div>
                <div className="col-6 flex justify-content-end">
                    <>
                        <Button label="add" icon="pi pi-plus" onClick={() => setShowCreateDialog(true)} role="dynaLoader-add-button" />
            ...
            <DynaLoaderEditDialogComponent entity={_.find(data, { _id: selectedEntityIndex })} show={showEditDialog} onHide={() => setShowEditDialog(false)} onEditResult={onEditResult} />
            <DynaLoaderCreateDialogComponent data={data} entity={newRecord} show={showCreateDialog} onHide={() => setShowCreateDialog(false)} onCreateResult={onCreateResult} />
            <DynaLoaderFakerDialogComponent show={showFakerDialog} onHide={() => setShowFakerDialog(false)} onFakerCreateResults={onFakerCreateResults} />
            <DynaLoaderSeederDialogComponent show={showSeederDialog} onHide={() => setShowSeederDialog(false)} onSeederResults={onSeederResults} />
