...
const DynaLoaderDataTable = ({
    ...
    return (
        <>
            ...
            >
                <Column field="from" header="Source" body={pTemplate0} filter={selectedFilterFields.includes('from')} hidden={selectedHideFields?.includes('from')} sortable style={{ minWidth: '8rem' }} />
                <Column field="to2" header="Destination" body={pTemplate1} filter={selectedFilterFields.includes('to2')} hidden={selectedHideFields?.includes('to2')} sortable style={{ minWidth: '8rem' }} />
                <Column field="name" header="Action" body={pTemplate2} filter={selectedFilterFields.includes('name')} hidden={selectedHideFields?.includes('name')} sortable style={{ minWidth: '8rem' }} />
            ...
            <Dialog header="Upload DynaLoader Data" visible={showUpload} onHide={() => setShowUpload(false)}>
                <UploadService />
            </Dialog>

            <Dialog header="Search DynaLoader" visible={searchDialog} onHide={() => setSearchDialog(false)}>
                Search
            </Dialog>
