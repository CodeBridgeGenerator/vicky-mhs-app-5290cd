...
import dynaLoaderFakerFactory from './dynaLoaderFakerFactory';
...
const DynaLoaderFakerDialogComponent = (props) => {
    ...
    const onRun = async () => {
        let fakeData = dynaLoaderFakerFactory(props.user, fakerCount);

        setLoading(true);
        const promises = fakeData.map((elem) => client.service('dynaLoader').create(elem));
...
export default connect(mapState, mapDispatch)(DynaLoaderFakerDialogComponent);
