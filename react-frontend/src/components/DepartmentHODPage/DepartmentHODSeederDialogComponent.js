...
import seederData from './departmentHOD.seeder.json';
...
const DepartmentHODSeederDialogComponent = (props) => {
    ...
    const onRun = async () => {
        ...
        try {
            for (let i = 0; i < seederData.length; i++) {
                ...
                const res = await client.service('departmentHOD').create(_data);
                results = [...results, res];
            }
            props.onHide();
            props.alert({
...
export default connect(mapState, mapDispatch)(DepartmentHODSeederDialogComponent);
