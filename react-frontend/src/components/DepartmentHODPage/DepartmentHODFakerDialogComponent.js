...
import departmentHODFakerFactory from './departmentHODFakerFactory';
...
const DepartmentHODFakerDialogComponent = (props) => {
    ...
    const onRun = async () => {
        let fakeData = departmentHODFakerFactory(props.user, fakerCount, departmentIdIds, employeeIdIds);

        setLoading(true);
        const promises = fakeData.map((elem) => client.service('departmentHOD').create(elem));
...
export default connect(mapState, mapDispatch)(DepartmentHODFakerDialogComponent);
