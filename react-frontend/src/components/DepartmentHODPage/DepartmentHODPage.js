...
const DepartmentHODPage = (props) => {
    ...
    useEffect(() => {
        ...
        const service = _.find(config.services, { serviceName: 'departmentHOD' });
        const fields =
    ...
    const deleteAll = async () => {
        ...
        const promises = data.map((e) => client.service('departmentHOD').remove(e._id));
        await Promise.all(
            promises.map((p) =>
                p.catch((error) => {
                    props.alert({
        ...
        <div className="mt-5">
            <div className="grid">
                <div className="col-6 flex justify-content-start">
                    <h3 className="mb-0 ml-2">DepartmentHOD </h3>
                </div>
                <div className="col-6 flex justify-content-end">
                    <>
                        <Button label="add" loading={loading} icon="pi pi-plus" onClick={() => setShowCreateDialog(true)} role="departmentHOD-add-button" />
            ...
            <div className="grid align-items-center">
                <div className="col-12" role="departmentHOD-datatable">
            ...
            <DepartmentHODEditDialogComponent entity={_.find(data, { _id: selectedEntityIndex })} show={showEditDialog} onHide={() => setShowEditDialog(false)} onEditResult={onEditResult} />
            <DepartmentHODCreateDialogComponent entity={newRecord} show={showCreateDialog} onHide={() => setShowCreateDialog(false)} onCreateResult={onCreateResult} />
            <DepartmentHODFakerDialogComponent show={showFakerDialog} onHide={() => setShowFakerDialog(false)} onFakerCreateResults={onFakerCreateResults} />
            <DepartmentHODSeederDialogComponent show={showSeederDialog} onHide={() => setShowSeederDialog(false)} onSeederResults={onSeederResults} />
