...
const DepartmentHODDataTable = ({
    ...
    const dropdownTemplate1 = (rowData, { rowIndex }) => <p>{rowData.employeeId?.name}</p>;
    ...
    return (
        <>
            <DataTable
                ...
                loading={loading}
            >
                <Column field="departmentId" header="DepartmentId" body={dropdownTemplate0} filter={selectedFilterFields.includes('departmentId')} hidden={selectedHideFields?.includes('departmentId')} style={{ minWidth: '8rem' }} />
                <Column field="employeeId" header="EmployeeId" body={dropdownTemplate1} filter={selectedFilterFields.includes('employeeId')} hidden={selectedHideFields?.includes('employeeId')} style={{ minWidth: '8rem' }} />
            ...
            <Dialog header="Upload DepartmentHOD Data" visible={showUpload} onHide={() => setShowUpload(false)}>
                <UploadService />
            </Dialog>

            <Dialog header="Search DepartmentHOD" visible={searchDialog} onHide={() => setSearchDialog(false)}>
                Search
            </Dialog>
...
export default DepartmentHODDataTable;
