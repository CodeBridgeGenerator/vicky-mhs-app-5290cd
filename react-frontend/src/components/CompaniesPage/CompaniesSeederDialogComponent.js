...
import seederData from './companies.seeder.json';
...
const CompaniesSeederDialogComponent = (props) => {
    ...
    const onRun = async () => {
        ...
        try {
            for (let i = 0; i < seederData.length; i++) {
                ...
                const res = await client.service('companies').create(_data);
                results = [...results, res];
            }
            props.onHide();
            props.alert({
...
export default connect(mapState, mapDispatch)(CompaniesSeederDialogComponent);
