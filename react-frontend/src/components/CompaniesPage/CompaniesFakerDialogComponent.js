...
import companiesFakerFactory from './companiesFakerFactory';
...
const CompaniesFakerDialogComponent = (props) => {
    ...
    useEffect(() => {
        ...
        client
            ...
            .then((data) => {
                setaddressesIds(data.data);
            });
    }, [props.show]);

    const onRun = async () => {
        let fakeData = companiesFakerFactory(props.user, fakerCount, addressesIds);

        setLoading(true);
        const promises = fakeData.map((elem) => client.service('companies').create(elem));
...
export default connect(mapState, mapDispatch)(CompaniesFakerDialogComponent);
