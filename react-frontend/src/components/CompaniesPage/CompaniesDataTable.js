...
const CompaniesDataTable = ({
    ...
    const p_numberTemplate2 = (rowData, { rowIndex }) => <p>{rowData.newCompanyNumber}</p>;
    const p_calendarTemplate3 = (rowData, { rowIndex }) => <p>{rowData.DateIncorporated}</p>;
    const dropdownTemplate4 = (rowData, { rowIndex }) => <p>{rowData.addresses?.Street1}</p>;
    const p_booleanTemplate5 = (rowData, { rowIndex }) => <p>{String(rowData.isdefault)}</p>;
    ...
    return (
        <>
            ...
            >
                ...
                <Column field="companyNo" header="Company no" body={pTemplate1} filter={selectedFilterFields.includes('companyNo')} hidden={selectedHideFields?.includes('companyNo')} sortable style={{ minWidth: '8rem' }} />
                <Column field="newCompanyNumber" header="New company number" body={p_numberTemplate2} filter={selectedFilterFields.includes('newCompanyNumber')} hidden={selectedHideFields?.includes('newCompanyNumber')} sortable style={{ minWidth: '8rem' }} />
                <Column field="DateIncorporated" header="Date Incorporated" body={p_calendarTemplate3} filter={selectedFilterFields.includes('DateIncorporated')} hidden={selectedHideFields?.includes('DateIncorporated')} sortable style={{ minWidth: '8rem' }} />
                <Column field="addresses" header="Addresses" body={dropdownTemplate4} filter={selectedFilterFields.includes('addresses')} hidden={selectedHideFields?.includes('addresses')} style={{ minWidth: '8rem' }} />
                <Column field="isdefault" header="Is default" body={p_booleanTemplate5} filter={selectedFilterFields.includes('isdefault')} hidden={selectedHideFields?.includes('isdefault')} sortable style={{ minWidth: '8rem' }} />
            ...
            <Dialog header="Upload Companies Data" visible={showUpload} onHide={() => setShowUpload(false)}>
                <UploadService />
            </Dialog>

            <Dialog header="Search Companies" visible={searchDialog} onHide={() => setSearchDialog(false)}>
                Search
            </Dialog>
