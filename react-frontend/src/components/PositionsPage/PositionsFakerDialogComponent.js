...
import positionsFakerFactory from './positionsFakerFactory';
...
const PositionsFakerDialogComponent = (props) => {
    ...
    const onRun = async () => {
        let fakeData = positionsFakerFactory(props.user, fakerCount, roleIdIds);

        setLoading(true);
        const promises = fakeData.map((elem) => client.service('positions').create(elem));
...
export default connect(mapState, mapDispatch)(PositionsFakerDialogComponent);
