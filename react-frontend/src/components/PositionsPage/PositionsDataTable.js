...
const PositionsDataTable = ({
    ...
    const dropdownTemplate0 = (rowData, { rowIndex }) => <p>{rowData.roleId?.name}</p>;
    const pTemplate1 = (rowData, { rowIndex }) => <p>{rowData.name}</p>;
    const inputTextareaTemplate2 = (rowData, { rowIndex }) => <p>{rowData.description}</p>;
    ...
    return (
        <>
            ...
            >
                <Column field="roleId" header="Role" body={dropdownTemplate0} filter={selectedFilterFields.includes('roleId')} hidden={selectedHideFields?.includes('roleId')} style={{ minWidth: '8rem' }} />
                ...
                <Column field="description" header="Description" body={inputTextareaTemplate2} filter={selectedFilterFields.includes('description')} hidden={selectedHideFields?.includes('description')} sortable style={{ minWidth: '8rem' }} />
                <Column field="abbr" header="Abbr" body={pTemplate3} filter={selectedFilterFields.includes('abbr')} hidden={selectedHideFields?.includes('abbr')} sortable style={{ minWidth: '8rem' }} />
                <Column field="isDefault" header="Is default" body={p_booleanTemplate4} filter={selectedFilterFields.includes('isDefault')} hidden={selectedHideFields?.includes('isDefault')} sortable style={{ minWidth: '8rem' }} />
            ...
            <Dialog header="Upload Positions Data" visible={showUpload} onHide={() => setShowUpload(false)}>
                <UploadService />
            </Dialog>

            <Dialog header="Search Positions" visible={searchDialog} onHide={() => setSearchDialog(false)}>
                Search
            </Dialog>
