...
import userAddressesFakerFactory from './userAddressesFakerFactory';
...
const UserAddressesFakerDialogComponent = (props) => {
    ...
    const onRun = async () => {
        let fakeData = userAddressesFakerFactory(props.user, fakerCount, userIdIds);

        setLoading(true);
        const promises = fakeData.map((elem) => client.service('userAddresses').create(elem));
...
export default connect(mapState, mapDispatch)(UserAddressesFakerDialogComponent);
