...
import staffinfoFakerFactory from './staffinfoFakerFactory';
...
const StaffinfoFakerDialogComponent = (props) => {
    ...
    const onRun = async () => {
        let fakeData = staffinfoFakerFactory(props.user, fakerCount);

        setLoading(true);
        const promises = fakeData.map((elem) => client.service('staffinfo').create(elem));
...
export default connect(mapState, mapDispatch)(StaffinfoFakerDialogComponent);
