...
import permissionServicesFakerFactory from './permissionServicesFakerFactory';
...
const PermissionServicesFakerDialogComponent = (props) => {
    const [fakerCount, setFakerCount] = useState(1);
    const [errors, setErrors] = useState([]);
    const [loading, setLoading] = useState(false);
    const [profileIds, setprofileIds] = useState();
    const [roleIdIds, setroleIdIds] = useState();
    const [positionIdIds, setpositionIdIds] = useState();
    const [employeeIdIds, setemployeeIdIds] = useState();
    ...
    useEffect(() => {
        ...
        client
            ...
            .then((data) => {
                setprofileIds(data.data);
            });
        client
            .service('roles')
            .find({ query: { $select: ['_id', 'name'] } })
            .then((data) => {
                setroleIdIds(data.data);
            });
        client
            .service('positions')
            .find({ query: { $select: ['_id', 'name'] } })
            .then((data) => {
                setpositionIdIds(data.data);
            });
        client
            .service('employees')
            .find({ query: { $select: ['_id', 'name'] } })
            .then((data) => {
                setemployeeIdIds(data.data);
            });
        client
            .service('users')
            .find({ query: { $select: ['_id', 'name'] } })
    ...
    const onRun = async () => {
        let fakeData = permissionServicesFakerFactory(props.user, fakerCount, profileIds, roleIdIds, positionIdIds, employeeIdIds, userIdIds);

        setLoading(true);
        const promises = fakeData.map((elem) => client.service('permissionServices').create(elem));
...
export default connect(mapState, mapDispatch)(PermissionServicesFakerDialogComponent);
