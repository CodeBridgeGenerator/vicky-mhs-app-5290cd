...
import JobQuesDatatable from './JobQuesDataTable1';
import JobQuesEditDialogComponent from './JobQuesEditDialogComponent';
...
const JobQuesPage = (props) => {
    ...
    const filename = 'jobQues.csv';

    useEffect(() => {
        let service = _.find(config.services, { serviceName: 'jobQues' });
        if (!service) {
            service = _.find(standard.services, { serviceName: 'jobQues' });
        }
    ...
    const deleteRow = async () => {
        try {
            await client.service('jobQues').remove(selectedEntityIndex);
    ...
    const deleteAll = async () => {
        ...
        const promises = data.map((e) => client.service('jobQues').remove(e._id));
        await Promise.all(
            promises.map((p) =>
                p.catch((error) => {
                    props.alert({
        ...
        <div className="mt-5">
            <div className="grid">
                <div className="col-6 flex justify-content-start">
                    <h3 className="mb-0 ml-2">Job Ques </h3>
                </div>
            ...
            <JobQuesEditDialogComponent entity={_.find(data, { _id: selectedEntityIndex })} show={showEditDialog} onHide={() => setShowEditDialog(false)} onEditResult={onEditResult} />
            <JobQuesCreateDialogComponent entity={newRecord} show={showCreateDialog} onHide={() => setShowCreateDialog(false)} onCreateResult={onCreateResult} />
            <JobQuesFakerDialogComponent show={showFakerDialog} onHide={() => setShowFakerDialog(false)} onFakerCreateResults={onFakerCreateResults} />
            <JobQuesSeederDialogComponent show={showSeederDialog} onHide={() => setShowSeederDialog(false)} onSeederResults={onSeederResults} />
