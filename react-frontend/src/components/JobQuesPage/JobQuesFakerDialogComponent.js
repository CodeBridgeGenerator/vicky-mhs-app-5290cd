...
import jobQuesFakerFactory from './jobQuesFakerFactory';
...
const JobQuesFakerDialogComponent = (props) => {
    const [fakerCount, setFakerCount] = useState(1);
    const [errors, setErrors] = useState([]);
    const [loading, setLoading] = useState(false);
    const [dynaLoaderIdIds, setdynaLoaderIdIds] = useState();
    ...
    useEffect(() => {
        ...
        client
            ...
            .then((data) => {
                setdynaLoaderIdIds(data.data);
            });
    }, [props.show]);

    const onRun = async () => {
        let fakeData = jobQuesFakerFactory(props.user, fakerCount, dynaLoaderIdIds);

        setLoading(true);
        const promises = fakeData.map((elem) => client.service('jobQues').create(elem));
...
export default connect(mapState, mapDispatch)(JobQuesFakerDialogComponent);
