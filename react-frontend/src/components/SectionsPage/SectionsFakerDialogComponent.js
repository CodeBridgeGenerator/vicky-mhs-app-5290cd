...
import sectionsFakerFactory from './sectionsFakerFactory';
...
const SectionsFakerDialogComponent = (props) => {
    ...
    const onRun = async () => {
        let fakeData = sectionsFakerFactory(props.user, fakerCount, departmentIdIds);

        setLoading(true);
        const promises = fakeData.map((elem) => client.service('sections').create(elem));
...
export default connect(mapState, mapDispatch)(SectionsFakerDialogComponent);
