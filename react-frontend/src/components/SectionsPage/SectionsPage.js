...
const SectionsPage = (props) => {
    ...
    useEffect(() => {
        let service = _.find(config.services, { serviceName: 'sections' });
        if (!service) {
            service = _.find(standard.services, { serviceName: 'sections' });
        }
    ...
    useEffect(() => {
        ...
        const service = _.find(config.services, { serviceName: 'sections' });
        const fields =
        ...
        <div className="mt-5">
            <div className="grid">
                <div className="col-6 flex justify-content-start">
                    <h3 className="mb-0 ml-2">Sections </h3>
                </div>
                <div className="col-6 flex justify-content-end">
                    <>
                        <Button label="add" icon="pi pi-plus" onClick={() => setShowCreateDialog(true)} role="sections-add-button" />
            ...
            <div className="grid align-items-center">
                <div className="col-12" role="sections-datatable">
                    <SectionsDatatable
                        items={data}
                        fields={fields}
            ...
            <SectionsEditDialogComponent entity={_.find(data, { _id: selectedEntityIndex })} show={showEditDialog} onHide={() => setShowEditDialog(false)} onEditResult={onEditResult} />
            <SectionsCreateDialogComponent entity={newRecord} show={showCreateDialog} onHide={() => setShowCreateDialog(false)} onCreateResult={onCreateResult} />
            <SectionsFakerDialogComponent show={showFakerDialog} onHide={() => setShowFakerDialog(false)} onFakerCreateResults={onFakerCreateResults} />
            <SectionsSeederDialogComponent show={showSeederDialog} onHide={() => setShowSeederDialog(false)} onSeederResults={onSeederResults} />
