...
import chataiFakerFactory from './chataiFakerFactory';
...
const ChataiFakerDialogComponent = (props) => {
    ...
    const onRun = async () => {
        let fakeData = chataiFakerFactory(fakerCount);

        setLoading(true);
        const promises = fakeData.map((elem) => client.service('chatai').create(elem));
...
export default connect(mapState, mapDispatch)(ChataiFakerDialogComponent);
