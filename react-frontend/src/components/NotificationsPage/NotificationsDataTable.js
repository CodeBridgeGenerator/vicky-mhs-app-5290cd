...
const NotificationsDataTable = ({
    ...
    const dropdownTemplate0 = (rowData, { rowIndex }) => <p>{rowData.toUser?.name}</p>;
    const pTemplate1 = (rowData, { rowIndex }) => <p>{rowData.content}</p>;
    const p_booleanTemplate2 = (rowData, { rowIndex }) => <p>{String(rowData.read)}</p>;
    const calendar_12Template3 = (rowData, { rowIndex }) => <p>{new Date(rowData.sent).toLocaleDateString()}</p>;
    ...
    return (
        <>
            <DataTable
                ...
                loading={loading}
            >
                <Column field="toUser" header="ToUser" body={dropdownTemplate0} filter={selectedFilterFields.includes('toUser')} hidden={selectedHideFields?.includes('toUser')} style={{ minWidth: '8rem' }} />
                <Column field="content" header="Content" body={pTemplate1} filter={selectedFilterFields.includes('content')} hidden={selectedHideFields?.includes('content')} sortable style={{ minWidth: '8rem' }} />
                <Column field="read" header="Read" body={p_booleanTemplate2} filter={selectedFilterFields.includes('read')} hidden={selectedHideFields?.includes('read')} style={{ minWidth: '8rem' }} />
                <Column field="sent" header="Sent" body={calendar_12Template3} filter={selectedFilterFields.includes('sent')} hidden={selectedHideFields?.includes('sent')} sortable style={{ minWidth: '8rem' }} />
            ...
            <Dialog header="Upload Notifications Data" visible={showUpload} onHide={() => setShowUpload(false)}>
                <UploadService />
            </Dialog>

            <Dialog header="Search Notifications" visible={searchDialog} onHide={() => setSearchDialog(false)}>
                Search
            </Dialog>
