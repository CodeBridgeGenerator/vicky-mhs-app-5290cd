...
import NotificationsFakerDialogComponent from './NotificationsFakerDialogComponent';
import NotificationsSeederDialogComponent from './NotificationsSeederDialogComponent';
...
const NotificationsPage = (props) => {
    ...
    useEffect(() => {
        ...
        const service = _.find(config.services, { serviceName: 'notifications' });
        const fields =
    ...
    const deleteRow = async () => {
        try {
            await client.service('notifications').remove(selectedEntityIndex);
    ...
    const deleteAll = async () => {
        ...
        const promises = data.map((e) => client.service('notifications').remove(e._id));
        await Promise.all(
            promises.map((p) =>
                p.catch((error) => {
                    props.alert({
    ...
    const onRowClick = ({ data }) => {
        navigate(`/notifications/${data._id}`);
    };
        ...
        <div className="mt-5">
            <div className="grid">
                <div className="col-6 flex justify-content-start">
                    <h3 className="mb-0 ml-2">Notifications </h3>
                </div>
                <div className="col-6 flex justify-content-end">
                    <>
                        <Button label="add" loading={loading} icon="pi pi-plus" onClick={() => setShowCreateDialog(true)} role="notifications-add-button" disabled={true} />
            ...
            <div className="grid align-items-center">
                <div className="col-12" role="notifications-datatable">
            ...
            <NotificationsEditDialogComponent entity={_.find(data, { _id: selectedEntityIndex })} show={showEditDialog} onHide={() => setShowEditDialog(false)} onEditResult={onEditResult} />
            <NotificationsCreateDialogComponent entity={newRecord} show={showCreateDialog} onHide={() => setShowCreateDialog(false)} onCreateResult={onCreateResult} />
            <NotificationsFakerDialogComponent show={showFakerDialog} onHide={() => setShowFakerDialog(false)} onFakerCreateResults={onFakerCreateResults} />
            <NotificationsSeederDialogComponent show={showSeederDialog} onHide={() => setShowSeederDialog(false)} onSeederResults={onSeederResults} />
