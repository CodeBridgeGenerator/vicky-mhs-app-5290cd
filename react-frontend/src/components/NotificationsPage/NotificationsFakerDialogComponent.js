...
import notificationsFakerFactory from './notificationsFakerFactory';
...
const NotificationsFakerDialogComponent = (props) => {
    ...
    useEffect(() => {
        ...
        client
            ...
            .then((data) => {
                settoUserIds(data.data);
            });
    }, [props.show]);

    const onRun = async () => {
        let fakeData = notificationsFakerFactory(props.user, fakerCount, toUserIds);

        setLoading(true);
        const promises = fakeData.map((elem) => client.service('notifications').create(elem));
...
export default connect(mapState, mapDispatch)(NotificationsFakerDialogComponent);
