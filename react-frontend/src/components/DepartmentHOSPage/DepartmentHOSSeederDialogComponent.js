...
import seederData from './departmentHOS.seeder.json';
...
const DepartmentHOSSeederDialogComponent = (props) => {
    ...
    const onRun = async () => {
        ...
        try {
            for (let i = 0; i < seederData.length; i++) {
                ...
                const res = await client.service('departmentHOS').create(_data);
                results = [...results, res];
            }
            props.onHide();
            props.alert({
...
export default connect(mapState, mapDispatch)(DepartmentHOSSeederDialogComponent);
