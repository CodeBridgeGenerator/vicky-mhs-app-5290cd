...
import departmentHOSFakerFactory from './departmentHOSFakerFactory';
...
const DepartmentHOSFakerDialogComponent = (props) => {
    const [fakerCount, setFakerCount] = useState(1);
    const [errors, setErrors] = useState([]);
    const [loading, setLoading] = useState(false);
    const [sectionIdIds, setsectionIdIds] = useState();
    ...
    const onRun = async () => {
        let fakeData = departmentHOSFakerFactory(props.user, fakerCount, sectionIdIds, employeeIdIds);

        setLoading(true);
        const promises = fakeData.map((elem) => client.service('departmentHOS').create(elem));
...
export default connect(mapState, mapDispatch)(DepartmentHOSFakerDialogComponent);
