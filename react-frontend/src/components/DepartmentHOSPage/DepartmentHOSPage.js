...
const DepartmentHOSPage = (props) => {
    ...
    useEffect(() => {
        ...
        client
            ...
            .find({
                query: {
                    $limit: 10000,
                    sectionId: urlParams.singleSectionsId,
        ...
        const service = _.find(config.services, { serviceName: 'departmentHOS' });
        const fields =
    ...
    const deleteAll = async () => {
        ...
        const promises = data.map((e) => client.service('departmentHOS').remove(e._id));
        await Promise.all(
            promises.map((p) =>
                p.catch((error) => {
                    props.alert({
        ...
        <div className="mt-5">
            <div className="grid">
                <div className="col-6 flex justify-content-start">
                    <h3 className="mb-0 ml-2">DepartmentHOS </h3>
                </div>
                <div className="col-6 flex justify-content-end">
                    <>
                        <Button label="add" loading={loading} icon="pi pi-plus" onClick={() => setShowCreateDialog(true)} role="departmentHOS-add-button" />
            ...
            <div className="grid align-items-center">
                <div className="col-12" role="departmentHOS-datatable">
            ...
            <DepartmentHOSEditDialogComponent entity={_.find(data, { _id: selectedEntityIndex })} show={showEditDialog} onHide={() => setShowEditDialog(false)} onEditResult={onEditResult} />
            <DepartmentHOSCreateDialogComponent entity={newRecord} show={showCreateDialog} onHide={() => setShowCreateDialog(false)} onCreateResult={onCreateResult} />
            <DepartmentHOSFakerDialogComponent show={showFakerDialog} onHide={() => setShowFakerDialog(false)} onFakerCreateResults={onFakerCreateResults} />
            <DepartmentHOSSeederDialogComponent show={showSeederDialog} onHide={() => setShowSeederDialog(false)} onSeederResults={onSeederResults} />
