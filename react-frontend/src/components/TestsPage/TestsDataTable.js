...
const TestsDataTable = ({
    ...
    const pTemplate0 = (rowData, { rowIndex }) => <p>{rowData.stack}</p>;
    const pTemplate1 = (rowData, { rowIndex }) => <p>{rowData.service}</p>;
    const badgeTemplate2 = (rowData, { rowIndex }) => <Badge value={Number(rowData.passed)} />;
    const badgeTemplate3 = (rowData, { rowIndex }) => <Badge value={Number(rowData.failed)} />;
    const pTemplate4 = (rowData, { rowIndex }) => <p>{rowData.notes}</p>;
    ...
    return (
        <>
            ...
            >
                <Column field="stack" header="Stack" body={pTemplate0} filter={selectedFilterFields.includes('stack')} hidden={selectedHideFields?.includes('stack')} sortable style={{ minWidth: '8rem' }} />
                ...
                <Column field="passed" header="Passed" body={badgeTemplate2} filter={selectedFilterFields.includes('passed')} hidden={selectedHideFields?.includes('passed')} sortable style={{ minWidth: '8rem' }} />
                <Column field="failed" header="Failed" body={badgeTemplate3} filter={selectedFilterFields.includes('failed')} hidden={selectedHideFields?.includes('failed')} sortable style={{ minWidth: '8rem' }} />
                <Column field="notes" header="Notes" body={pTemplate4} filter={selectedFilterFields.includes('notes')} hidden={selectedHideFields?.includes('notes')} sortable style={{ minWidth: '8rem' }} />
            ...
            <Dialog header="Upload Tests Data" visible={showUpload} onHide={() => setShowUpload(false)}>
                <UploadService />
            </Dialog>

            <Dialog header="Search Tests" visible={searchDialog} onHide={() => setSearchDialog(false)}>
                Search
            </Dialog>
