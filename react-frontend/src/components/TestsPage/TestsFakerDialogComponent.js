...
import testsFakerFactory from './testsFakerFactory';
...
const TestsFakerDialogComponent = (props) => {
    ...
    const onRun = async () => {
        let fakeData = testsFakerFactory(props.user, fakerCount);

        setLoading(true);
        const promises = fakeData.map((elem) => client.service('tests').create(elem));
...
export default connect(mapState, mapDispatch)(TestsFakerDialogComponent);
