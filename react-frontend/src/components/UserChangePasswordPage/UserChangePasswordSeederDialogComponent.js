...
import seederData from './userChangePassword.seeder.json';
...
const UserChangePasswordSeederDialogComponent = (props) => {
    ...
    const onRun = async () => {
        ...
        try {
            for (let i = 0; i < seederData.length; i++) {
                ...
                const res = await client.service('userChangePassword').create(_data);
                results = [...results, res];
            }
            props.onHide();
            props.alert({
...
export default connect(mapState, mapDispatch)(UserChangePasswordSeederDialogComponent);
