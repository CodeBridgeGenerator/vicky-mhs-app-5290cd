...
import userChangePasswordFakerFactory from './userChangePasswordFakerFactory';
...
const UserChangePasswordFakerDialogComponent = (props) => {
    ...
    const onRun = async () => {
        let fakeData = userChangePasswordFakerFactory(props.user, fakerCount);

        setLoading(true);
        const promises = fakeData.map((elem) => client.service('userChangePassword').create(elem));
...
export default connect(mapState, mapDispatch)(UserChangePasswordFakerDialogComponent);
