...
const ErrorsWHDataTable = ({
    ...
    const p_calendarTemplate0 = (rowData, { rowIndex }) => <p>{rowData.date}</p>;
    const pTemplate1 = (rowData, { rowIndex }) => <p>{rowData.details}</p>;
    ...
    return (
        <>
            <DataTable
                ...
                loading={loading}
            >
                <Column field="date" header="Date" body={p_calendarTemplate0} filter={selectedFilterFields.includes('date')} hidden={selectedHideFields?.includes('date')} sortable style={{ minWidth: '8rem' }} />
                <Column field="details" header="Details" body={pTemplate1} filter={selectedFilterFields.includes('details')} hidden={selectedHideFields?.includes('details')} style={{ minWidth: '8rem' }} />
            ...
            <Dialog header="Upload ErrorsWH Data" visible={showUpload} onHide={() => setShowUpload(false)}>
                <UploadService />
            </Dialog>

            <Dialog header="Search ErrorsWH" visible={searchDialog} onHide={() => setSearchDialog(false)}>
                Search
            </Dialog>
...
export default ErrorsWHDataTable;
