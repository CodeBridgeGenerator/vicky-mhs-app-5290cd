...
import errorsWHFakerFactory from './errorsWHFakerFactory';
...
const ErrorsWHFakerDialogComponent = (props) => {
    ...
    const onRun = async () => {
        let fakeData = errorsWHFakerFactory(props.user, fakerCount);

        setLoading(true);
        const promises = fakeData.map((elem) => client.service('errorsWH').create(elem));
...
export default connect(mapState, mapDispatch)(ErrorsWHFakerDialogComponent);
