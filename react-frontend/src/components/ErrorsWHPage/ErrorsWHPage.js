...
import ErrorsWHDatatable from './ErrorsWHDataTable';
...
import ErrorsWHFakerDialogComponent from './ErrorsWHFakerDialogComponent';
import ErrorsWHSeederDialogComponent from './ErrorsWHSeederDialogComponent';

const ErrorsWHPage = (props) => {
    ...
    useEffect(() => {
        ...
        const service = _.find(config.services, { serviceName: 'errorsWH' });
        const fields =
    ...
    const deleteRow = async () => {
        try {
            await client.service('errorsWH').remove(selectedEntityIndex);
    ...
    const deleteAll = async () => {
        ...
        const promises = data.map((e) => client.service('errorsWH').remove(e._id));
        await Promise.all(
            promises.map((p) =>
                p.catch((error) => {
                    props.alert({
        ...
        <div className="mt-5">
            <div className="grid">
                <div className="col-6 flex justify-content-start">
                    <h3 className="mb-0 ml-2">ErrorsWH </h3>
                </div>
                <div className="col-6 flex justify-content-end">
                    <>
                        <Button label="add" loading={loading} icon="pi pi-plus" onClick={() => setShowCreateDialog(true)} role="errorsWH-add-button" disabled={true} />
            ...
            <div className="grid align-items-center">
                <div className="col-12" role="errorsWH-datatable">
                    <ErrorsWHDatatable
                        items={data}
                        fields={fields}
            ...
            <ErrorsWHEditDialogComponent entity={_.find(data, { _id: selectedEntityIndex })} show={showEditDialog} onHide={() => setShowEditDialog(false)} onEditResult={onEditResult} />
            <ErrorsWHCreateDialogComponent entity={newRecord} show={showCreateDialog} onHide={() => setShowCreateDialog(false)} onCreateResult={onCreateResult} />
            <ErrorsWHFakerDialogComponent show={showFakerDialog} onHide={() => setShowFakerDialog(false)} onFakerCreateResults={onFakerCreateResults} />
            <ErrorsWHSeederDialogComponent show={showSeederDialog} onHide={() => setShowSeederDialog(false)} onSeederResults={onSeederResults} />
...
export default connect(mapState, mapDispatch)(ErrorsWHPage);
