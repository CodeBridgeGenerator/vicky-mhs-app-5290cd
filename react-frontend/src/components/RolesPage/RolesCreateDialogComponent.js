...
const RolesCreateDialogComponent = (props) => {
    ...
    const onSave = async () => {
        ...
        try {
            const result = await client.service('roles').create(_data);
            props.onHide();
            props.alert({
                type: 'success',
                title: 'Create info',
                message: 'Info Roles created successfully'
            });
            ...
            props.alert({
                type: 'error',
                title: 'Create',
                message: 'Failed to create in Roles'
            });
        }
        setLoading(false);
    };
    ...
    return (
        <Dialog header="Create Roles" visible={props.show} closable={false} onHide={props.onHide} modal style={{ width: '40vw' }} className="min-w-max" footer={renderFooter()} resizable={false}>
            <div className="grid p-fluid overflow-y-auto" style={{ maxWidth: '55vw' }} role="roles-create-dialog-component">
