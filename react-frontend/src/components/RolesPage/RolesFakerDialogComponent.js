...
import rolesFakerFactory from './rolesFakerFactory';
...
const RolesFakerDialogComponent = (props) => {
    ...
    const onRun = async () => {
        let fakeData = rolesFakerFactory(props.user, fakerCount);

        setLoading(true);
        const promises = fakeData.map((elem) => client.service('roles').create(elem));
...
export default connect(mapState, mapDispatch)(RolesFakerDialogComponent);
