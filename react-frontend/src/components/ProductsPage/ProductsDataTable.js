...
const ProductsDataTable = ({ items, fields, onEditRow, onRowDelete, onRowClick, searchDialog, setSearchDialog,   showUpload, setShowUpload,
...
const currencyTemplate1 = (rowData, { rowIndex }) => <InputNumber value={rowData.price}  mode="currency" currency="MYR" locale="en-US" disabled={true} />
    ...
    const paginatorRight = DownloadCSV({ data : items, fileName : "products"});
...
<Column field="price" header="Price" body={currencyTemplate1} filter={selectedFilterFields.includes("price")} hidden={selectedHideFields?.includes("price")}  sortable style={{ minWidth: "8rem" }} />
        ...
        <Dialog header="Upload Products Data" visible={showUpload} onHide={() => setShowUpload(false)}>
        <UploadService />
      </Dialog>

      <Dialog header="Search Products" visible={searchDialog} onHide={() => setSearchDialog(false)}>
      Search
    </Dialog>
    <Dialog
        header="Filter Users"
