...
import productsFakerFactory from "./productsFakerFactory";
...
const ProductsFakerDialogComponent = (props) => {
    ...
    const onRun = async () => {
        let fakeData = productsFakerFactory(props.user,fakerCount);

        setLoading(true);
        const promises = fakeData.map((elem) => client.service("products").create(elem));
...
export default connect(mapState, mapDispatch)(ProductsFakerDialogComponent);
