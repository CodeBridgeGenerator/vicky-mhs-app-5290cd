...
const MailQuesDataTable = ({
    ...
    const inputTextareaTemplate6 = (rowData, { rowIndex }) => <p>{rowData.content}</p>;
    ...
    return (
        <>
            ...
            >
                ...
                <Column field="from" header="From" body={pTemplate1} filter={selectedFilterFields.includes('from')} hidden={selectedHideFields?.includes('from')} sortable style={{ minWidth: '8rem' }} />
                <Column field="recipients" header="Recipients" body={chipTemplate2} filter={selectedFilterFields.includes('recipients')} hidden={selectedHideFields?.includes('recipients')} sortable style={{ minWidth: '8rem' }} />
                <Column field="status" header="Status" body={tickTemplate3} filter={selectedFilterFields.includes('status')} hidden={selectedHideFields?.includes('status')} style={{ minWidth: '8rem' }} />
                <Column field="templateId" header="Template" body={pTemplate4} filter={selectedFilterFields.includes('templateId')} hidden={selectedHideFields?.includes('templateId')} style={{ minWidth: '8rem' }} />
                <Column field="subject" header="Subject" body={inputTextareaTemplate5} filter={selectedFilterFields.includes('subject')} hidden={selectedHideFields?.includes('subject')} sortable style={{ minWidth: '8rem' }} />
                <Column field="jobId" header="Job Id" body={p_numberTemplate7} filter={selectedFilterFields.includes('jobId')} hidden={selectedHideFields?.includes('jobId')} sortable style={{ minWidth: '8rem' }} />
                <Column field="errors" header="Errors" body={inputTextareaTemplate8} filter={selectedFilterFields.includes('errors')} hidden={selectedHideFields?.includes('errors')} sortable style={{ minWidth: '8rem' }} />
                <Column field="end" header="End" body={p_calendarTemplate9} filter={selectedFilterFields.includes('end')} hidden={selectedHideFields?.includes('end')} sortable style={{ minWidth: '8rem' }} />
        ...
        <Column header="Delete" body={deleteTemplate} /> */}
            ...
            <Dialog header="Upload MailQues Data" visible={showUpload} onHide={() => setShowUpload(false)}>
                <UploadService />
            </Dialog>

            <Dialog header="Search MailQues" visible={searchDialog} onHide={() => setSearchDialog(false)}>
                Search
            </Dialog>
