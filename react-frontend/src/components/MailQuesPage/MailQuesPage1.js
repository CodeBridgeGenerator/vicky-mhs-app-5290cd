...
import MailQuesDatatable from './MailQuesDataTable1';
...
import MailQuesFakerDialogComponent from './MailQuesFakerDialogComponent';
import MailQuesSeederDialogComponent from './MailQuesSeederDialogComponent';
...
const MailQuesPage = (props) => {
    ...
    useEffect(() => {
        ...
        const service = _.find(config.services, { serviceName: 'mailQues' });
        const fields =
    ...
    const deleteRow = async () => {
        try {
            await client.service('mailQues').remove(selectedEntityIndex);
    ...
    const deleteAll = async () => {
        ...
        const promises = data.map((e) => client.service('mailQues').remove(e._id));
        await Promise.all(
            promises.map((p) =>
                p.catch((error) => {
                    props.alert({
        ...
        <div className="mt-5">
            <div className="grid">
                <div className="col-6 flex justify-content-start">
                    <h3 className="mb-0 ml-2">MailQues </h3>
                </div>
                <div className="col-6 flex justify-content-end">
                    <>
                        <Button label="add" loading={loading} icon="pi pi-plus" onClick={() => setShowCreateDialog(true)} role="mailQues-add-button" disabled={true} />
            ...
            <MailQuesEditDialogComponent entity={_.find(data, { _id: selectedEntityIndex })} show={showEditDialog} onHide={() => setShowEditDialog(false)} onEditResult={onEditResult} />
            <MailQuesCreateDialogComponent entity={newRecord} show={showCreateDialog} onHide={() => setShowCreateDialog(false)} onCreateResult={onCreateResult} />
            <MailQuesFakerDialogComponent show={showFakerDialog} onHide={() => setShowFakerDialog(false)} onFakerCreateResults={onFakerCreateResults} />
            <MailQuesSeederDialogComponent show={showSeederDialog} onHide={() => setShowSeederDialog(false)} onSeederResults={onSeederResults} />
