...
import mailQuesFakerFactory from './mailQuesFakerFactory';
...
const MailQuesFakerDialogComponent = (props) => {
    const [fakerCount, setFakerCount] = useState(1);
    const [errors, setErrors] = useState([]);
    const [loading, setLoading] = useState(false);
    const [templateIdIds, settemplateIdIds] = useState();
    ...
    useEffect(() => {
        ...
        client
            ...
            .then((data) => {
                settemplateIdIds(data.data);
            });
    }, [props.show]);

    const onRun = async () => {
        let fakeData = mailQuesFakerFactory(props.user, fakerCount, templateIdIds);

        setLoading(true);
        const promises = fakeData.map((elem) => client.service('mailQues').create(elem));
...
export default connect(mapState, mapDispatch)(MailQuesFakerDialogComponent);
