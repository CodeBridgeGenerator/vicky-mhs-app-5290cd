...
import seederData from './permissionFields.seeder.json';
...
const PermissionFieldsSeederDialogComponent = (props) => {
    ...
    const onRun = async () => {
        ...
        try {
            for (let i = 0; i < seederData.length; i++) {
                ...
                const res = await client.service('permissionFields').create(_data);
                results = [...results, res];
            }
            props.onHide();
            props.alert({
...
export default connect(mapState, mapDispatch)(PermissionFieldsSeederDialogComponent);
