...
import permissionFieldsFakerFactory from './permissionFieldsFakerFactory';
...
const PermissionFieldsFakerDialogComponent = (props) => {
    const [fakerCount, setFakerCount] = useState(1);
    const [errors, setErrors] = useState([]);
    const [loading, setLoading] = useState(false);
    const [servicePermissionIdIds, setservicePermissionIdIds] = useState();
    ...
    useEffect(() => {
        ...
        client
            ...
            .find({ query: { $select: ['_id', 'service'] } })
            .then((data) => {
                setservicePermissionIdIds(data.data);
            });
    }, [props.show]);

    const onRun = async () => {
        let fakeData = permissionFieldsFakerFactory(props.user, fakerCount, servicePermissionIdIds);

        setLoading(true);
        const promises = fakeData.map((elem) => client.service('permissionFields').create(elem));
...
export default connect(mapState, mapDispatch)(PermissionFieldsFakerDialogComponent);
